...
\begin{table}[htbp]
    ...
    \begin{tabular}{l p{10.5cm}}
        ...
        Velocitat de l'enllaç de xarxa & 1~Gbps (Adaptador virtualitzat per VMware) \\
    \end{tabular}
...
\begin{itemize}
    \item \textbf{Temps de Desplegament ($T_{deploy}$):} Interval temporal absolut requerit per aprovisionar el node. Inclou des de l'inici de l'execució del \textit{playbook} d'Ansible fins que els contenidors (o \textit{Pods}) assoleixen l'estat operatiu.
    
    \item \textbf{Rendiment Global (\textit{Throughput}):} Avalua la capacitat de la xarxa i el processador sota estrès. Es defineix com la quantitat d'imatges (tensors) processades amb èxit per unitat de temps. Si $N$ és el nombre total d'imatges enviades i $T_{total}$ és l'interval de temps en segons des de la primera petició fins a l'última resposta:
        $$ Throughput = \frac{N}{T_{total}} \quad \text{[img/s]} $$
    
    \item \textbf{Consum Mitjà de Recursos (RAM i CPU):} El consum base (en repòs) s'avalua capturant $k$ mostres discretes del monitoratge del sistema (mitjançant \texttt{ps} o cgroups) durant un període d'estabilització. La petjada de memòria es calcula com la mitjana aritmètica d'aquestes lectures:
        $$ \overline{RAM}_{rep\text{ò}s} = \frac{1}{k} \sum_{i=1}^{k} RAM_i \quad \text{[MB]} $$    \item \textbf{Temps Mitjà de Recuperació (MTTR):} Temps requerit pel sistema (en mil·lisegons) per restablir el servei després de provocar la terminació forçada del procés principal mitjançant senyals de caiguda abrupta.
