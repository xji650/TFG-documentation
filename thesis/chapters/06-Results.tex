...
\begin{table}[h]
    ...
    \midrule
    T\_deploy (Aprovisionament) & 218,67~s & 145,16~s \\
    Recuperació (MTTR) & 452,09~ms & 1.487,28~ms \\
    CPU Repòs & <0,1~\% & 0,80~\% \\
    RAM Repòs (Sobrecost Estructural) & 722,50~MB & 834,40~MB \\
    Throughput d'Injecció & 9.053,76~img/s & 6.269,21~img/s \\
    \bottomrule
    \end{tabular}
...
Cal destacar que, pel que fa a l'aprovisionament (T\_deploy), K3s desplega més ràpid que Systemd (145,16 s enfront de 218,67 s). Això s'explica per la capacitat del Kubelet de K3s per paral·lelitzar i gestionar eficientment l'extracció de les imatges de la seva memòria cau interna a través del containerd, en contrast amb el procés seqüencial de càrrega d'artefactes aïllats que imposa l'automatització via Ansible a l'entorn natiu.
