...
La Taula \ref{tab:resultados_rendimiento_red} presenta les mètriques consolidades, extretes de la mitjana històrica del \textit{Data Lake} (9 execucions) després de sotmetre els nodes a càrrega extrema.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|}
        ...
        \hline
        HTTP / JSON & 0.54 & 1661.81 & 277.63 & 268.00 & 219.41 & 10.39 \\
        \hline
        gRPC / Protobuf & 0.16 & 5747.52 & 149.08 & 132.42 & 207.74 & 1.99 \\
        \hline
        ZeroMQ / Protobuf & 0.14 & 6199.34 & 141.70 & 131.86 & 183.74 & 1.99 \\
        \hline
        ZeroMQ / MessagePack & 0.15 & 6052.72 & 143.14 & 132.39 & 186.86 & 1.99 \\
        \hline
    \end{tabular}%
    }
...
La transició cap a gRPC i Protobuf resol els problemes crítics del \textit{baseline}. La càrrega de xarxa es redueix al seu pes matemàtic pur (1,99 MB). Aquesta eficiència permet utilitzar tècniques de \textit{Zero-Copy} (\texttt{np.frombuffer}) a la memòria cau, reduint el temps de processament local a la meitat (132 ms) i disparant el \textit{throughput} per sobre de les 5.700 img/s.

\textbf{3. Eficiència de Memòria i el Límit del Transport (ZeroMQ)}  
Tot i l'alta velocitat de gRPC, el seu motor intern basat en HTTP/2 afegeix un \textit{overhead} de capçaleres innecessari. Substituir-lo per connexions TCP crues (\textit{Brokerless}) amb ZeroMQ assoleix el límit físic de velocitat. Tal com s'il·lustra a la Figura \ref{fig:comparativa_protocols_ram}, les arquitectures basades en ZeroMQ presenten la petjada de memòria (\textit{RAM Footprint}) més baixa sota estrès (183-186 MB), aconseguint un estalvi crític per a dispositius \textit{Edge} limitats en comparació amb l'ecosistema gRPC.
...
L'objectiu d'aquesta comparativa és quantificar l'"Impost Arquitectònic" (\textit{Architectural Overhead}) que introdueix Kubernetes al ser desplegat en hardware de recursos limitats enfront de solucions d'execució nativa.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|c|}
    \hline
    \textbf{Mètrica} & \textbf{Systemd (Natiu)} & \textbf{Kubernetes (K3s)} \\ \hline
    T\_deploy (Aprovisionament) & 198.19 s & 145.62 s \\ \hline
    Recuperació (MTTR) & 468.27 ms & 1574.13 ms \\ \hline
    CPU Repòs & 0.00 \% & 0.67 \% \\ \hline
    RAM Repòs (Impost Arquitectònic) & 722.72 MB & 835.28 MB \\ \hline
    \end{tabular}
    \caption{Comparativa de rendiment i infraestructura: Protocol 04-zeromq-messagepack (9 tests).}
...
Avaluant el comportament habitual de la infraestructura, Systemd demostra una velocitat de recuperació superior i altament estable, reiniciant el servei en una mitjana de ~468 ms. Kubernetes, per la seva banda, requereix una mitjana de ~1.574 ms (més del triple de temps), tal com reflecteix la Figura \ref{fig:mttr_resiliencia}. Tècnicament, Systemd gestiona els processos directament a nivell de \textit{kernel} com a dimonis locals, la qual cosa permet un reinici gairebé instantani. Kubernetes opera sota un model distribuït asíncron: el bucle de reconciliació (\textit{Reconciliation Loop}) ha de detectar la caiguda a través de l'API, el \textit{Scheduler} ha de reprogramar el \textit{Pod}, i el \textit{Kubelet} ha d'interactuar amb el motor de contenidors i sol·licitar una nova IP virtual al CNI, afegint latència inherent al procés.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        \hline
        \textbf{Mitjana (\textit{Mean})} & 468 ms & 1.574 ms \\
        \textbf{Mínim Registrat} & 455 ms & 1.480 ms \\
        \textbf{Màxim Registrat} & 495 ms & 1.685 ms \\
        \textbf{Desviació Estàndard Aproximada ($\sigma$)} & $\pm$ 15 ms & $\pm$ 75 ms \\
        \hline
    \end{tabular}
